...
function connectDB(done) {
  if (mongoose.connection.name !== 'discoveredm') {
    return done();
  }

  mongoose.connect((process.env.MONGO_URL || 'mongodb://localhost:27017/discoveredm'), function (err) {
...
describe('GET /api/getPosts', function () {
  ...
  beforeEach('connect and add two post entries', function (done) {
    ...
    connectDB(function () {
      var post1 = new Post({name: 'Prashant', title: 'Hello EDM', content: "All cats meow!"});
      var post2 = new Post({name: 'Mayank', title: 'Hi EDM', content: "All dogs bark!"});
